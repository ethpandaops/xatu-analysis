    ...
    try:
        ...
        if clear_cache:
            conflicting_modules = [
                ...
                'plot_generators', 'interactive_dashboard', 'beaconchain_integration',
                'rated_integration', 'performance_summary', 'report_generator', 'session_state', 'time_utils'
            ]
...
try:
    ...
    with isolated_import_context(current_dir):
        ...
        module_name = f"validator_performance_dashboard_{id(current_dir)}"
    ...
    st.error(f"Failed to load Validator Performance dashboard: {e}")
