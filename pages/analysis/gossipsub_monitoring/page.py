...
def isolated_import_context(directory_path, clear_cache=True):
    ...
    try:
        ...
        if clear_cache:
            conflicting_modules = [
                'config_utils', 'data_loaders', 'plot_generators', 
                'interactive_dashboard', 'queries'
            ]
...
try:
    ...
    with isolated_import_context(current_dir):
        ...
        module_name = f"gossipsub_dashboard_{id(current_dir)}"
    ...
    st.error(f"Failed to load Gossipsub Monitoring dashboard: {e}")
