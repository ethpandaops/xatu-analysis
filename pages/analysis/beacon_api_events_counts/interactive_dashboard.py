...
def render_sidebar_config(cluster: str, network: str) -> Dict[str, Any]:
    ...
    if selected_period != "Custom":
        ...
        start_date = start_col1.date_input(
            "Start Date",
            value=min(st.session_state.get('beapi_start_date_custom', default_start.date()), datetime.now().date()),
        ...
        end_date = end_col1.date_input(
            "End Date",
            value=min(st.session_state.get('beapi_end_date_custom', default_end.date()), datetime.now().date()),
            max_value=datetime.now().date(),
