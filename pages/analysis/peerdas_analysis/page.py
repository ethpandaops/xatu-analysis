...
def isolated_import_context(directory_path, clear_cache=True):
    ...
    try:
        ...
        if clear_cache:
            conflicting_modules = [
                'config_utils', 'plot_generators', 'interactive_dashboard',
                'loader', 'queries'
            ]
...
try:
    ...
    with isolated_import_context(current_dir):
        ...
        module_name = f"peerdas_dashboard_{id(current_dir)}"
    ...
    st.error(f"Failed to load PeerDAS Analysis dashboard: {e}")
